      ...
      @csrf
            ...
            <div class="grid gap-4 sm:grid-cols-2 sm:gap-6 ">
              <div class="sm:col-span-2">
                  ...
                  <input type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-black  text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type product name" required="">
                  <br>
                  <label for="capacity" class="text-white block mb-2 text-sm font-medium  dark:text-white">Capacity</label>
                  <input type="number" name="capacity" id="capacity" class="bg-gray-50 border border-gray-300 text-black  text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type Room capacity"  required="">
              </div>
    ...
    <table class=" w-full text-sm text-left rtl:text-right text-blue-100 dark:text-blue-00">
        ...
        @foreach($ExaminationRooms as $ExaminationRoom)
            <tr class="bg-indigo-600 border-b border-blue-400">
                ...
                <td class="px-6 py-4">
                    <a href="{{ route('modify_R', ['id' => $ExaminationRoom->id]) }}" class="hover:text-green-400 font-medium text-white hover:underline">Edit</a>
                </td>
                <td class="px-6 py-4">
                    ...
                    <form  method="POST" action="{{route('delete_R', ['id' => $ExaminationRoom->id])}}">
                        @csrf
                        @method("DELETE")
                        <input class="hover:text-red-600 font-medium bg-indigo-600 text-white hover:underline" type="submit" value="DELETE" />
                    </form>
                </td>
            </tr>
            @endforeach
